...
# load UIEB images
uieb_images = list(sorted(Path(f'{UIEB_PATH}/train/reference/').glob('*.png'), key=lambda x: int(x.stem)))
print(uieb_images[START_IDX:START_IDX+TRAIN_SIZE])

def load_pair(idx):
    light = cv2.imread(str(uieb_images[idx]))[..., ::-1]
    dark = cv2.imread(f'{UIEB_PATH}/train/raw/{uieb_images[idx].name}')[..., ::-1]
...
def test_model(model):
    # read image
    l = cv2.imread(f'{UIEB_PATH}/val/raw/{uieb_images[VAL_IMG_INDEX].name}')[..., ::-1] / 255.0
    d = cv2.imread(f'{UIEB_PATH}/val/raw/{uieb_images[VAL_IMG_INDEX].name}')[..., ::-1] / 255.0
    # l = cv2.imread(f'{UIEB_PATH}/val5/high/45.png')[..., ::-1] / 255.0
    # d = cv2.imread(f'{UIEB_PATH}/val5/low/45.png')[..., ::-1] / 255.0
    ...
    z = torch.from_numpy(x).float().unsqueeze(0)  # add batch dimension

    # sample colors from the model
    model.eval()
    with torch.no_grad():  # no need to compute gradients
        colors = model.sample(z.to(device)).cpu()

    # convert to numpy array
    colors = colors.squeeze(0).permute(1, 2, 0).numpy()

    # normalization
    colors = np.maximum(colors, 0)
    colors = colors / (np.sum(colors, axis=-1, keepdims=True) + 1e-8)

    # convert colors to an image format
    colors_image = (colors * 255).astype(np.uint8)

    # save the image
    save_path = f'{UIEB_PATH}/val/processed/{uieb_images[VAL_IMG_INDEX].name}'
    cv2.imwrite(save_path, colors_image[..., ::-1])  # convert from RGB to BGR for OpenCV
...
def train():
    ...
    for epoch in range(1, 1001):
        ...
        for i, (x, y) in enumerate(dl):
            rand_x = randint(0, 128)
            rand_y = randint(0, 128)
            x = x[:, :, rand_y:rand_y+128, rand_x:rand_x+128]
            y = y[:, :, rand_y:rand_y+128, rand_x:rand_x+128]
